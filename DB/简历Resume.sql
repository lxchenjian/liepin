...
-- 	TO_SECONDS(r.refresh_time) as refreshTimeSeconds,
-- 	(TO_SECONDS(now()) - TO_SECONDS(r.refresh_time)) as justActiveSeconds
-- 	SELECT TIMESTAMPDIFF(SECOND,'2021-06-04 14:02:36','2021-06-04 14:10:57');
FROM 
	resume r
LEFT JOIN 
	users u
...
LEFT JOIN
	(SELECT 
		...
		from 
				...
				-- 	count(edu.user_id) over(partition by edu.user_id ORDER BY edu.end_date desc) as counts 
				from 
...
-- 		(SELECT 
-- 				workExp.resume_id as resumeId,
-- 				workExp.user_id as userId,
-- 				workExp.company_name as companyName,
-- 				workExp.position as position,
-- 				workExp.industry as industry,
-- 				workExp.begin_date as beginDate,
-- 				workExp.end_date as endDate
-- 			FROM 
-- 				resume_work_exp workExp
-- 			ORDER BY 
-- 				workExp.end_date DESC
-- 			LIMIT 0,1) as latest_company
ON
...
FROM 
	resume_work_exp workExp
WHERE
	workExp.user_id = '1541299724478681090'
ORDER BY 
	workExp.end_date DESC
LIMIT 0,1;
